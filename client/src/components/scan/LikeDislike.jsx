...
function LikeDislike({ addressId }) {
    ...
    return (
        <div className="relative flex flex-col border rounded-xl bg-background/60 p-4 gap-3 justify-center items-center">
            ...
            <div>
                <Button onClick={() => handleVote(1)} variant="ghost"><ThumbsUp className="text-success" />{likes}</Button>
                <Button onClick={() => handleVote(-1)} variant="ghost"><ThumbsDown className="text-destructive" />{dislikes}</Button>
            </div>
