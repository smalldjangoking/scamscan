...
class ReportSchema(BaseModel):
    ...
    def both_subjects_not_allowed(cls, data):
        ...
        if data.subject == "website" and (
            data.crypto_address is not None or data.crypto_name is not None
        ):
            raise ValueError(
                "crypto_address and crypto_name must be None when subject is 'website'"
            )
        return data
