...
async def user_create(
    request: Request, user: UserRegistrationSchema, session: SessionDep
):
    """Registration"""
    ...
    try:
        ...
        return {
            "status": "ok",
            "detail": "User created successfully. Please check your email.",
        }
        ...
        raise HTTPException(
            status_code=500,
            detail="Something went wrong. Please, repeat in few minutes",
        )
...
async def user_login(
    userbase: UserLoginSchema, session: SessionDep, response: Response
):
    """
    Login
    ...
    if not user.is_active:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Your account is not active"
        )

    if not user.is_email_verified:
        old_token = await session.scalar(
            select(Email_tokens).where(Email_tokens.user_id == user.id)
        )
        ...
        raise HTTPException(
            ...
            detail="Your email is not verified yet. We've sent you a verification link",
        )
...
async def web_refresh_token_validator(request: Request, response: Response):
    ...
    try:
        ...
        payload = jwt.decode(
            refresh_token, os.getenv("SECRET_KEY"), algorithms=[ALGORITHM]
        )
...
@router.patch(
    "/email/verify/{token}", status_code=status.HTTP_200_OK, include_in_schema=False
)
async def verify_email(token: str, session: SessionDep):
    ...
    if result.expires_at < datetime.now(timezone.utc):
        ...
        raise HTTPException(
            ...
            detail="Token is expired. Please log in to request a new confirmation email.",
        )
...
@router.patch(
    "/password/verify/{token}", status_code=status.HTTP_200_OK, include_in_schema=False
)
async def password_change(
    session: SessionDep,
    token: str = Path(
        ..., description="Token from email to confirm your rights to the account"
    ),
) -> dict:
    token_db = await session.execute(
        select(Email_tokens).where(Email_tokens.token == token)
    )
...
@router.patch(
    "/password/change/{token}", status_code=status.HTTP_200_OK, include_in_schema=False
)
...
async def restore_password_token_create(
    payload: EmailRequest, session: SessionDep
) -> dict:
